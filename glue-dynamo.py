...
def upsert_item(row):
    try:
        # Attempt toparse and insert/update data in DynamoDB
        track_id = row['track_id']
        report_date = row['report_date']
        total_listens = int(row['total_listens'])
        unique_users = int(row['unique_users'])
        total_listening_time = Decimal(row['total_listening_time'])
        avg_listening_time_per_user = Decimal(row['avg_listening time_per_user'])
